...
class ChartyInstance(object):
    ...
    # Subnet ID for my VPC
    SUBNETID = 'subnet-0a6525a25b0b848aa'
    ...
    def create(self):
        ...
        self._instance = self._ec2.create_instances(
            ImageId=self.AMI,
            InstanceType=self.EC2_INSTANCE_TYPE,
            MinCount=1,
            MaxCount=1,
            NetworkInterfaces=[{'SubnetId': self.SUBNETID, 'DeviceIndex': 0, 'AssociatePublicIpAddress': True, 'Groups': [self._security_group_id]}])
        for instance in self._instance:
            instance.wait_until_running()
            instance.reload()
            
        # Get instance information after it has been created.   
        logging.info("Created instance '{}' with IPAddress '{}'".format(instance.id, instance.public_ip_address))

        self._public_ip_address = instance.public_ip_address
    ...
    def connectSSH(self):
        ...
        while not connected and retries_remaining > 0:
            try:
                logging.info("connecting to {}@{} via SSH using key file named '{}'".format(self.USERNAME, self._public_ip_address, self.KEY_FILE_NAME))
                client.connect(hostname=self._public_ip_address, username=self.USERNAME, pkey=paramiko.RSAKey.from_private_key_file(self.KEY_FILE_NAME), allow_agent=False, look_for_keys=False)
                connected = True
    ...
    def putCredentials(self):
        ...
        # TODO
        try:
            command = subprocess.check_output("sudo apt-get install -y git-core", stderr=subprocess.STDOUT)
        ...
        if command == 0:
            ...
            print("Failed with code: %i" % command)
    ...
    def runRemoteCommand(self, command):
        ...
        try:
            testcommand = subprocess.check_output("echo This is a test!", stderr=subprocess.STDOUT)
        except OSError as err:
            print("Caught OSError")
            output = err
        except subprocess.CalledProcessError as err:
    ...
    def installCharty(self):
        ...
        # ssh over to Virtual box and run ./install.sh in the following directory ~/Code/lib
        try:
            command = subprocess.check_output("./ChartyProjectAutomation.sh", stderr=subprocess.STDOUT)
        except OSError as err:
            print("Caught OSError")
            output = err
        except subprocess.CalledProcessError as err:
        ...
        if command == 0:
            logging.info('charty is available at http://{}:{}/'.format(self._public_ip_address, self.CHARTY_TCP_PORT))
        else:
            print("Failed with code: %i" % command)
